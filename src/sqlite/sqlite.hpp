...
namespace sqlite{
	...
	public:
		...
		std::array<variant, n> next(){
			...
			if(error != "not an error" && error != "no more rows available" && error != "another row available" && error != "column index out of range"){
				log(*getSqliteLog(), "    error: \"", error, "\"");
			}
			...
			if(sqlite3_column_count(handle) == n){
				for(int i = 0; i < n; i++){
					...
					switch(type){
						...
						case SQLITE_NULL:{
							row[i] = std::monostate();
							log(*getSqliteLog(), "    error: type == null");
						} break;
						default: {
							log(*getSqliteLog(), "    error: unknown type ", type);
						}
					}
				}
			}
			else{
				log(*getSqliteLog(), "    error: unexpected number of columns: ", sqlite3_column_count(handle), ", expected ", n);
			}
